...
export class JugadorService {
  ...
  protected convertDateFromClient(jugador: IJugador): IJugador {
    return Object.assign({}, jugador, {
      nacimiento: jugador.nacimiento?.isValid() ? jugador.nacimiento.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.nacimiento = res.body.nacimiento ? dayjs(res.body.nacimiento) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((jugador: IJugador) => {
        jugador.nacimiento = jugador.nacimiento ? dayjs(jugador.nacimiento) : undefined;
      });
    }
    return res;
  }
